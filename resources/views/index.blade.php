...
<head>
    ...
    <title>Sistema para Colas UGEL HUANUCO</title>
    <!-- CSS files -->
        ...
        <div class="container-fluid">
            <div class="row">
                ...
                <div class="col s12 center-align">
                    <h4>Por favor, ingrese su Documento de identidad</h4>
                </div>
            </div>
            <div ></div>
            <div class="row">
                    ...
                    <form action="{{ route('save.client') }}" method="POST" class="card grey lighten-5 hoverable">
                        ...
                        <div class="card-content row ">
                            <div class="input-field col s12">
                                <input type="text" pattern="[0-9]+" title="Solo puedes usar números" id="ci" minlength="5" maxlength="8" name="ci" style="height:80px; font-size:40px" >   //auth()->user() ? (auth()->user()->tickets->name_ticket == 'REGISTRADOR' || auth()->user()->tickets->name_ticket == 'SUPER USUARIO'? '' : 'required disabled="true"') : 'required disabled="true"'}}         
                                @if ($errors->has('ci'))
                                    <p>{{ $errors->first('ci') }}</p>
                                @endif
